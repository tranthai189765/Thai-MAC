"""Actor-critic networks for the executor and the coordinator.

* :class:`A3C_Single` -- the **executor**: encodes the targets/obstacles seen by
  a single camera with self-attention and outputs a continuous (pan, zoom)
  action via a Gaussian policy.
* :class:`A3C_Multi` -- the **coordinator**: encodes all camera-target pairs,
  attends over them and outputs a discrete on/off goal-assignment per pair. Its
  critic can use a Shapley-value decomposition (:class:`AMCValueNet`) for
...
from .modules import NoisyLinear, AttentionLayer
from .utils import norm_col_init
from . import observation as obs


def build_model(env, args, device):
    """Instantiate the executor or coordinator network from ``args.model``."""
    name = args.model
    if 'single' in name:
        ...
        raise ValueError(f"Unknown model '{name}': expected a 'single' or 'multi' variant.")
...
def sample_action(mu_multi, sigma_multi, device, test=False):
    """Sample (or take the argmax of) a discrete action from logits ``mu_multi``."""
    logit = mu_multi
...
class PolicyNet_Coor(nn.Module):
    """Discrete goal-assignment head for the coordinator."""
...
class PolicyNet(nn.Module):
    """Continuous (pan, zoom) control head for the executor (Gaussian policy)."""
...
class ValueNet(nn.Module):
    """Scalar state-value head for the executor."""
...
class ValueNet_Coor(nn.Module):
    """Scalar state-value head for the coordinator (non-Shapley variant)."""
...
class AMCValueNet(nn.Module):
    """Shapley-value critic for multi-agent credit assignment.

    The marginal contribution of each agent is estimated by attending over the
    coalition formed by the preceding agents; summing the marginal values
    yields the cooperative team value.
    """
    ...
    def forward(self, x, goal):
        ...
        # Marginal contribution of the first agent (empty preceding coalition).
        feature = torch.zeros([self.feature_dim]).to(self.device)
        value.append(self.critic_linear(torch.cat([feature, coalition[0]])))
        # Marginal contribution of each subsequent agent given its predecessors.
        for j in range(1, n):
...
class A3C_Single(torch.nn.Module):
    """Executor network: per-camera continuous (pan, zoom) control policy."""
    ...
    def forward(self, x, test=False):
        input_targets = obs.joint_camera_observation_over_targets(self.env, x)
        input_obstacles = obs.joint_camera_observation_over_obstacles(self.env, x)
...
class A3C_Multi(torch.nn.Module):
    """Coordinator network: discrete camera-to-target goal assignment policy."""
