"""Optimizers with shared state, suitable for asynchronous (A3C) training.

The optimizer statistics live in shared memory so that every worker process
applies updates against the same running moments.
"""
...
class SharedRMSprop(optim.Optimizer):
    """RMSprop with state buffers placed in shared memory."""

    def __init__(self, params, lr=7e-4, alpha=0.99, eps=0.1,
...
class SharedAdam(optim.Optimizer):
    """Adam (with optional AMSGrad) with state buffers placed in shared memory."""
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if amsgrad:
                    # Maintain the running maximum of the second moment.
